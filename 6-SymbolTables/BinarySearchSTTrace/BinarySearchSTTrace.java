...
public class BinarySearchSTTrace<Key extends Comparable<Key>, Value> {

  static int compareToCount;
  static int allComparesCount;
  static BinarySearchSTTrace st = new BinarySearchSTTrace<Character, Integer>();
  ...
  public static void main(String[] args) {
    ...
    for (char c : chars) {
      ...
      printState(c, i-1, compareToCount, allComparesCount);
    }

  }

  static void printState(char c, int value, int compareToCount, int allComparesCount) {
    String keysString = "";
    for (var key : st.keys) {
    ...
    for (var val : st.vals) {
      ...
      valsString += padLeft(val.toString(), 2) + " ";
    }
    System.out.println(
      padRight(c, 5) +
      padRight(value, 7) +
      padRight(keysString, 22) +
      padRight(valsString, 40) +
      padRight(compareToCount, 11) +
      padRight(allComparesCount, 10)
    );
  }
  ...
  /*
   * Unaltered part of BinarySearchSTTrace from algs4
   */

  /******************************************************************************
   * Compilation: javac BinarySearchSTTrace.java Execution: java
   * BinarySearchSTTrace Dependencies: StdIn.java StdOut.java Data files:
   ...
   * 
   * % java BinarySearchSTTrace < tinyST.txt A 8 C 4 E 12 H 5 L 11 M 9 P 10 R 3 S
   * 0 X 7
   *
   ******************************************************************************/
   ...
   */
  public BinarySearchSTTrace(int capacity) {
  ...
  private void resize(int capacity) {
    ...
    for (int i = 0; allComparesCount++>=0 && i < n; i++) {
      tempk[i] = keys[i];
      tempv[i] = vals[i];
    }
    vals = tempv;
  ...
  public int rank(Key key) {
    if (allComparesCount++>=0 && key == null)
      throw new IllegalArgumentException("argument to rank() is null");

    int lo = 0, hi = n - 1;
    while (allComparesCount++>=0 && lo <= hi) {
      int mid = lo + (hi - lo) / 2;
      ...
      if (allComparesCount++>=0 && cmp < 0) {
        hi = mid - 1;
      } else if (allComparesCount++>=0 && cmp > 0) {
        lo = mid + 1;
      } else
        return mid;
    }
  ...
  public void put(Key key, Value val) {
    ...
    if (allComparesCount++>=0 && val == null) {
      delete(key);
      return;
    }
    ...
    if (allComparesCount++>=0 && i < n) {
      compareToCount += 1;
      if (allComparesCount++>=0 && keys[i].compareTo(key) == 0) {
        vals[i] = val;
        return;
      }
    }
    ...
    if (allComparesCount++>=0 && n == keys.length)
      resize(2 * keys.length);

    for (int j = n; allComparesCount++>=0 && j > i; j--) {
      keys[j] = keys[j - 1];
      vals[j] = vals[j - 1];
    }
